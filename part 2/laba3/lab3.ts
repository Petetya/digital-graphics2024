...
class HomeOwner implements Owner {
...
// 1.2 Реализуйте интерфейс Транспортное средство (Vehicle) 
...
class HondaShine implements Motorbike{
    motorcycleFrame: string;
    isForSport: boolean;
    brand: string;
    model: string;
...
class MyVehicleStorage<T extends Vehicle> implements VehicleStorage<T>{
    createdAt: Date;
    vehicles: T[];
    getVehicles(): T[]{
